...
const getChequesByNegocio = async (negocioId, limit, page) => {
    try {
        ...
        const totalCheques = await prisma.cheque.count({ where: { negocioId: parseInt(negocioId) } });
        return {
            cheques,
            total: totalCheques,
            totalPages: Math.ceil(totalCheques / limit),
...
module.exports = { getAllCheques, getChequeById, getChequesByNegocio, addCheque, updateCheque, updateChequeStatus };